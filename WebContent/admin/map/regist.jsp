	...
	$.ajax({
		...
		success:function(result){
			...
			if(json.length<1){
				...
				var n=prompt(json.length+" 건이 발견되었습니다.\n 원하시는 산의 번호로 선택하세요[0~"+(json.length-1)+"]");
				obj=json[n];
				alert("결정된 산의 정보 : "+obj.detail);
				setData(obj);
			}
...
<div class="container">
  <form enctype="multipart/form-data"> <!-- 텍스트 뿐만아니라 모든 바이너리 데이터를 보낼 수 있음 -->

    <input type="text" name="name" placeholder="산 이름" style="width:85%">
    
    <input type="button" value="공공 데이터 검색">

    <input type="text" name="addr" placeholder="주소">

    <textarea name="detail" placeholder="상세정보" style="height:200px"></textarea>
    
    <input type="file" name="myFile"/><!-- DTO에서 MultipartFile 의 변수명과 일치해야 등록됨 -->
